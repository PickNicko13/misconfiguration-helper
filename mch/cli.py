...
async def async_scan(
	...
	with Live(progress, refresh_per_second=50) as live:
		tasks = {
			host: progress.add_task(f'{host}: waiting', total=len(scan_types))
		...
		for host in all_hosts:
			...
			logger.debug(
				f'Validated host: {validated_host} (type: {type(validated_host)})'
			)
			...
			for t in scan_types:
				...
				try:
					scanner = SCANNERS[t](
						validated_host, config, state_mgr, warn_html_errors
					)
				...
				while len(completed) < len(scanner_tasks):
					for t, scanner, task in scanner_tasks:
						...
						if task.done():
							...
							logger.debug(
								f'Final status update for {t} on {host}: complete, warnings={warnings}'
							)
							update_status(
								progress, tasks, host, status, warnings, len(errors)
							)
							live.refresh()
							progress.update(tasks[host], advance=1)
						else:
							if hasattr(scanner, 'get_progress'):
								...
								if progress_str != last_progress[t]:
									...
									logger.debug(
										f'Progress update for {t} on {host}: {progress_str}'
									)
									update_status(
										progress,
										tasks,
	...
	if not no_notify:
		...
		await send_notification(
			title='MCH Scan Complete', message=f'Scanned {len(all_hosts)} hosts.'
		)
	...
	for host in hosts:
		...
		if type == 'critical':
			acao_issues = [
				...
				if i['status'] == 'uncategorized'
				and i['weak_type'] in ['arbitrary', 'leaked_ip']
			]
			table.add_row(
				'ACAO Issues',
			...
			new_ports = [
				...
				if p not in state['ports'].get('acknowledged', [])
			]
			fuzz_issues = state['fuzz'].get('issues', []) + state['fuzz'].get(
				'will_fix', []
			)
			acao_issues = [
				i
				for i in state.get('acao', {}).get('issues', [])
				if i['status'] in ['uncategorized', 'will_fix']
			]
			table.add_row(
				'Unacked Ports',
				f'[yellow]{len(new_ports)} ports[/yellow]' if new_ports else 'None',
			)
			table.add_row(
				'Fuzz Issues',
				f'[yellow]{len(fuzz_issues)} found[/yellow]' if fuzz_issues else 'None',
			)
			table.add_row(
				'ACAO Issues',
				f'[yellow]{len(acao_issues)} issues[/yellow]'
				if acao_issues
				else 'None',
			)
		elif type == 'all':
		...
		if type == 'warnings':
			...
			if acao_issues:
				...
				for issue in sorted(
					...
					endpoint = (
						f'{issue["scheme"]}://{issue["hostname"]}{issue["endpoint"]}'
					)
					console.print(
						f'  - {endpoint} ({issue["weak_type"]}: {issue["detail"]})'
					)
...
def ack(host: str = typer.Argument(..., help='Host to acknowledge issues')):
	...
	for path in fuzz_issues:
		...
		if response in ['false_positive', 'wont_fix']:
			...
			console.print(
				f'[green]Marked {path} as {response.replace("_", "-")}[/green]'
			)
	...
	for issue in acao_issues:
		...
		if response in ['false_positive', 'wont_fix']:
			...
			console.print(
				f'[green]Marked {endpoint} as {response.replace("_", "-")}[/green]'
			)
